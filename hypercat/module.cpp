...
// Input  : *svModuleName
//-----------------------------------------------------------------------------
CModule::CModule(const string& svModuleName, std::uintptr_t moduleBase) : m_svModuleName(svModuleName), m_pModuleBase(moduleBase)
{
...
vector<CModule::ModuleSections_t> CModule::GetSections() const
{
...
CMemory CModule::FindFreeDataPage(const size_t sizeWanted)
{
	return nullptr; // DFS
}

void CModule::UnlinkFromPEB() // I have not done this in ages, I think that was right though. 
// Disclaimer: This does not bypass GetMappedFileName. That function calls NtQueryVirtualMemory which does a syscall to ntoskrnl for getting info on a section.
{
...
void GetModules()
{
	...
	g_sCachedModules["self.exe"] = CModule("self.exe", processEnvBlock->ImageBaseAddress);
...
#pragma warning( disable : 4996) // Codecvt is still not deprecated in c++20. Hence why we still use it. I don't wanna use the windows internal alternative.
		...
		std::string moduleName = std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(reinterpret_cast<wchar_t*>(pldrEntry->BaseDllName.Buffer));
#pragma warning( pop ) 
		g_sCachedModules[moduleName] = CModule(moduleName, baseAddr);
	}
}
